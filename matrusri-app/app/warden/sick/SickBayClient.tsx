...
import { addSickLog, markSickParentCalled, markSickRecovered } from "@/lib/actions";
...
export default function SickBayClient({ sickLogs, students }: { sickLogs: SickLog[]; students: Student[] }) {
  ...
  async function markRecovered(id: string, name?: string) {
    if (!confirm(`Mark ${name ?? "this student"} as recovered? They will be removed from the sick list.`)) return;
    await markSickRecovered(id);
    router.refresh();
  }
        ...
        return (
          <div
            ...
            className={`bg-white border border-slate-200 border-l-4 ${
              overOneHour ? "border-l-red-500" : "border-l-amber-500"
            } rounded-xl p-4 mb-3`}
          >
            ...
            <div className="mt-3 bg-slate-50 rounded p-2 text-xs space-y-1">
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {!parentCalled && (
                <button
                  ...
                  className="flex-1 min-w-[40%] bg-white text-blue-800 text-sm font-semibold py-2.5 rounded-lg border border-blue-800"
                >
                  📞 Primary called
                </button>
              )}
              {!emergencyCalled && s.student?.emergency_contact_name && (
                <button
              ...
              <button
                onClick={() => markRecovered(s.id, s.student?.name)}
                className="flex-1 min-w-[40%] bg-emerald-600 text-white text-sm font-semibold py-2.5 rounded-lg"
              >
                ✓ Recovered
              </button>
            </div>
            ...
            <div className="text-[11px] text-slate-400 text-center mt-3">
              If parent comes to pick up, use the <strong>New outing</strong> tab to create a pickup record.
            </div>
          </div>
        );
      })}
